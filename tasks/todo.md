...
- [ ] **B6 — Google Calendar read and availability**
  ...
  - **What is left for B6**, therefore: Google Calendar OAuth and free/busy read, deriving the group's common free windows, calling `meetsMinimumLength` on each one (too short → `stuck` with a reason), and calling `trimPairToViableSlots` per candidate. The eval adapter is already wired: `scenarioAgentInput` calls `filterTrimmedPairs`, which is the same call B6 makes from the calendar side.
