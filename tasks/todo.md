...
- [x] **A5 — Eval runner** — `npm run eval` prints pass rate, cost, duration, cycles and hard-constraint violations per scenario. Violations column must be zero.
  - **Every scenario that can be scored today has been measured: 5 scored, 5 passed, 0 hard-constraint violations, $0.021 in all, 6–23s per scenario.** `01`, `02`, `06` on 9 Sep; `03` and `05` on 16 Sep, once the trimming made them scorable — two requests, no retries. In both, the model kept leximin's first choice over a longer evening, and in `05` over a better-rated venue too. Full table and the decisions in [docs/decisions/eval-runner.md](../docs/decisions/eval-runner.md); the plan is [tasks/a5-plan.md](a5-plan.md).
  - ⚠️ **Those passes were measured with the answer's reasoning in the prompt** — the eval adapter passed each scenario's `description` as the occasion. Removed 16 Sep 2026 under A6; `03`, `05` and `06` re-measured without it and still pass. `01` and `02` have one legal pair each, so no prompt can change them. See [tasks/a6-plan.md](a6-plan.md).
  - ⚠️ **One live run per scenario** — nothing yet shows an answer is stable across runs, and one of seven duration samples is over §12's 20-second target (`06`, 22.9s). A10, which re-runs the eval set to compare models, is the natural place for a repeat.
  - **Three of eight scenarios are waiting on a stage, not on the model** (was five, until the trimming landed). `07` and `08` on **A7/A8**, and — found by the first sweep — `04` on **A12**: its own `expected.reasoning` says a straight-line-only engine is expected to pick the other venue, so without the Context Resolver's detour factor nothing could reach the agreed answer. The classification moved; the eval answer did not. `classify` in `evals/judge.ts` derives all of it from `trap` and `needsTrim`, so each becomes scored on its own when its stage lands.
...
- [x] **A6 — Per-participant justification quality**
  ...
  - Decisions, and what A7/A8, B11 and C6 inherit: [docs/decisions/justification-quality.md](../docs/decisions/justification-quality.md)
  ...
  - **Added 16 Sep 2026 — every justification is in Hebrew, and states only facts it was given.** Recorded runs showed identical sentences for people the model could not tell apart, and invented reasons ("arterial roads") where it had no data. The model is now told each person's dietary needs, allergies and lost travel modes, so it can name them; the prompt forbids invented facts and asks for gender-neutral Hebrew; code rejects a reason that is not Hebrew; `npm run eval` counts distinct explanations. Plan and research: [tasks/a6-plan.md](a6-plan.md).

### Milestone 2 (weeks 4–6)

- [x] **A7 — Constraint Updater: free-text rejection → structured constraint** — the project's central mechanism. `gemini-3.5-flash-lite` ([spec §6.4](../docs/spec.md)) — **not Haiku**, which this line named from before the 2026-08-26 provider change. Shares prompt, schema and validation conventions with A12.
  - What it writes: a **per-meeting correction** on the rejecting participant's `ParticipantMeetingContext`, in `SoftPreferences` vocabulary — never an edit to the profile. "Too expensive this time" does not make somebody permanently thrifty, and a per-meeting row records whose objection it was and which re-weighing it triggered. Decided on [#86](https://github.com/ron14y-sys/squad_lock/issues/86); scenarios `07` and `08` are the worked examples.
  - **Done 24 Sep 2026.** Plan: [tasks/a7-plan.md](a7-plan.md) · decisions: [docs/decisions/constraint-updater.md](../docs/decisions/constraint-updater.md).
  - **It did not depend on B11's column in the end — it added it.** `participant_meeting_contexts.softPreferences` and `responses.extractionOutcome` ship here, with their migration, applied. B11 inherits them rather than owning them.
  - **Four objections this vocabulary cannot hold are classified, not forced into a field**: "too far" and "too late" are A12's material, "not that place" is A8b's, and "no reason I can name" is a real answer. An invented soft preference would change the next ranking on an opinion nobody stated ([#86](https://github.com/ron14y-sys/squad_lock/issues/86)).
  - **A4 is shown the correction and the sentence it came from**, as two fields, never merged — a silent merge leaves the model nothing to point at, and success criterion 4 asks for a proposal that _visibly_ addresses the reason.
  - **Measured:** extraction 8/8, each 3/3 across three runs, $0.0047 over 24 calls (`npm run eval:constraints`). The corrected cycle reaches the agreed follow-up on both rejection scenarios (`npm run eval -- --followup`). Eight checks against the real database (`npm run verify:a7-db`).
  - ⚠️ **`07` and `08` are still `deferred` in `npm run eval`.** The follow-up harness is not the loop — no cap, no `stuck`, nothing persisted — and §12.5's pass rate may only carry the claim the product can make. **A8** is what moves them, and what deletes `scenarioFollowupInput`.
  - ⚠️ **How a cycle is counted is open** — [#125](https://github.com/ron14y-sys/squad_lock/issues/125). Today every `doesnt_suit` spends one, so three people rejecting the same proposal within a minute reach the cap having produced one corrected proposal or none. To decide in A8.
...
- [ ] **A10 — Model and effort check against the eval set** — can `gemini-3.5-flash-lite` hold six profiles without dropping one? Reduced from a full sweep. (Named Haiku until A7 corrected it: no `claude-*` model has been in this project since 2026-08-26, [spec §6.4](../docs/spec.md).)
...
- [x] **B6 — Google Calendar read and availability**
  ...
  - Three pieces, three PRs: `commonFreeWindows` / `slotTolerance` in `lib/matching/availability.ts` (merged in #115, `candidateSlots` since removed — see the resolved overlap below); the Google Calendar `freebusy.query` client, `fetchBusy`, in `lib/calendar/freebusy.ts` (merged in #116); the database wiring — `fetchBusyForUsers` / `fetchBusyForConnections`, reading `User.googleRefreshToken` — in `lib/calendar/participant-busy.ts` (merged in #117).
  - ✅ **The intersection with opening hours and reach is `trimPairToViableSlots`** in [`lib/matching/constraints.ts`](../lib/matching/constraints.ts) (#105). It takes one free window, a candidate, the participants and their distances, and returns every viable `(venue, slot)` the pair still supports — opening hours and reach both applied, the three-hour minimum enforced, an empty result meaning the pair is dropped. 12 tests, no LLM and no network. Decisions in [docs/decisions/slot-trimming.md](../docs/decisions/slot-trimming.md).
    - **Why it was carved out:** it has no Google dependency at all, and it was sitting behind an OAuth integration it does not use. It silently broke eval scenarios `03`, `05` and `07` — and in `05` and `07` the agreed answer was filtered out while a _different_ venue survived and was proposed. Found by A5's first sweep.
  - **The eval adapter is already wired**: `scenarioAgentInput` calls `filterTrimmedPairs`, the same trimming the calendar side needs, so eval scenarios `03` and `05` are scored with no Google dependency.
  - **The shortest meeting worth proposing is three hours**, applied in both places: a group free window under three hours is `stuck` with a reason (`meetsMinimumLength`), and a `(venue, time)` pair narrowed under three hours is dropped — one pair, not the run (decided on [#86](https://github.com/ron14y-sys/squad_lock/issues/86)).
  - **Mobility narrows a slot, it does not forbid a venue.** Done inside the trimming, and **narrowing is on reach, never on tolerance** — being further than someone is comfortable with is a burden leximin weighs, not a wall (scenario `06`'s agreed answer puts two people at 1.15× their tolerance). On foot the reach is about a kilometre; transit and car are uncapped ([#89](https://github.com/ron14y-sys/squad_lock/issues/89)). `slotTolerance` (#115) is the burden side of the same rule: `min(profile.toleranceKm, cap ?? Infinity)` as the leximin denominator. **Deterministic here, not in A12** — the Resolver ships dark and falls back, so a Resolver-only version hands a car-sized tolerance back to someone who cannot drive every time it is off.
  - A participant with no refresh token on file, or one Google has rejected, fails the **whole** availability computation rather than silently treating that person as free. Same reasoning as §5.7's conflict check: a false positive here is worse than a false negative.
  - ✅ **The two overlaps found when #105 met #115 are resolved.** The live funnel is `commonFreeWindows` → `trimPairToViableSlots` per candidate → `filterPairs` — `candidateSlots` cut a free window into fixed three-hour slots ahead of trimming, which left the trimming nothing to shorten (it keeps a pre-cut slot whole or drops it), throwing away what it exists for. `candidateSlots` and `MIN_MEETING_HOURS` are removed from `lib/matching/availability.ts`; `MINIMUM_MEETING_MINUTES` in `constraints.ts` is now the one place the three-hour rule lives. `commonFreeWindows` and `slotTolerance` are unaffected — nothing outside `availability.ts` called `candidateSlots` or `slotTolerance` yet (confirmed by grep), so this was a same-file cleanup, not a live behaviour change.
  - ⚠️ **No live caller yet.** Nothing runs the funnel end to end, because that needs real venues (B7) and whoever assembles a full `Participant[]` and runs a match (A4/B11).
...
- [ ] **B11 — Per-meeting context: persistence and batching** — ✅ **the column is done**: A7 added `participant_meeting_contexts.softPreferences` (nullable, no default — `NULL` is "no correction", `{}` would read as "corrected to nothing") and `responses.extractionOutcome`, with the migration applied. ⚠️ **Reading a context back is field-wise, not row-wise**: rows append, and an A7 correction row carries no origin while an amendment row carries no correction, so each field comes from the latest row that _has_ it. Write `ParticipantMeetingContext`; open a **~90-second batching window** that further amendments reset; the window is closed by the next feed poll, so **no cron and no background job** (spec §3.2). Verify two amendments 30 seconds apart produce exactly one run.
...
- [x] **C8b — The `stuck` state has a screen** — best option found, why it stopped, and a way for the group to settle it. It does not silently vanish from the feed. Merged in #135 (`StuckPanel.tsx`). A8 turned out to be an unbroken-down placeholder and B5 already moves a meeting to `stuck` at the cap, so nothing waited on it. Added `POST /api/meetings/[id]/cancel` (initiator only, stuck only): nothing in the app could otherwise close or cancel a meeting, so a stuck one would hold one of the group's three open slots forever.
