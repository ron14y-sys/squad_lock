...
- [x] **B6 — Google Calendar read and availability**
  ...
  - ⚠️ **The venue side of "intersected with venue opening hours" is A2's `filterPairs`, already built** — it already consumes exactly the `TimeSlot[]` this task produces (`ConstraintInput.slots`). What is still missing is a live caller: nothing yet runs `commonFreeWindows` → `candidateSlots` → `filterPairs` end to end, because that needs real venues (B7) and whoever assembles a full `Participant[]` and runs a match (A4).
