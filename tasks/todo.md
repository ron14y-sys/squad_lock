...
- [x] **B6 — Google Calendar read and availability**
  ...
  - Three pieces, three PRs: `commonFreeWindows` / `candidateSlots` / `slotTolerance` in `lib/matching/availability.ts` (merged in #115); the Google Calendar `freebusy.query` client, `fetchBusy`, in `lib/calendar/freebusy.ts` (merged in #116); the database wiring — `fetchBusyForUsers` / `fetchBusyForConnections`, reading `User.googleRefreshToken` — in `lib/calendar/participant-busy.ts` (merged in #117).
  ...
  - ⚠️ **Two open overlaps, found when #105 met #115 — to decide, not yet changed in code:**
    - **Two constants for one rule:** `MIN_MEETING_HOURS = 3` in `availability.ts` and `MINIMUM_MEETING_MINUTES = 180` in `constraints.ts`. They agree today; nothing keeps them agreeing.
    - **Two shapes of slot:** `candidateSlots` cuts a free window into fixed three-hour slots, one per hour; `trimPairToViableSlots` takes the whole free window and shortens it to fit each venue. Fed a three-hour slot, the trimming has nothing to shorten — it keeps the slot whole or drops it — so chaining them throws away what the trimming is for. The live funnel must pick one: `commonFreeWindows` → `candidateSlots` → `filterPairs`, or `commonFreeWindows` → `trimPairToViableSlots` per candidate → `filterPairs`.
