...
- [x] **A5 — Eval runner** — `npm run eval` prints pass rate, cost, duration, cycles and hard-constraint violations per scenario. Violations column must be zero.
  ...
  - **Five of eight scenarios are waiting on a stage, not on the model.** `03` and `05` on the trimming — the function now exists (see B6 below), and they become scored as soon as `evals/adapter.ts` calls it — `07` and `08` on **A7/A8**, and — found by the first sweep — `04` on **A12**: its own `expected.reasoning` says a straight-line-only engine is expected to pick the other venue, so without the Context Resolver's detour factor nothing could reach the agreed answer. The classification moved; the eval answer did not. `classify` in `evals/judge.ts` derives all of it from `trap` and `needsTrim`, so each becomes scored on its own when its stage lands.
...
- [x] **B6 — Google Calendar read and availability**
  ...
  - **The eval adapter does not call it yet** — `evals/adapter.ts` still hands over the untrimmed window, so `03`, `05` and `07` stay blocked until that wiring lands (#106).
