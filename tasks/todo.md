...
- [ ] **B6 — Google Calendar read and availability**
  ...
  - ✅ **The intersection is already built — B6 does not write it.** `trimPairToViableSlots` in [`lib/matching/constraints.ts`](../lib/matching/constraints.ts) takes one free window, a candidate, the participants and their distances, and returns every viable `(venue, slot)` the pair still supports — opening hours and reach both applied, the three-hour minimum enforced, an empty result meaning the pair is dropped. 12 tests, no LLM and no network. Decisions in [docs/decisions/slot-trimming.md](../docs/decisions/slot-trimming.md).
  ...
  - **What is left for B6**, therefore: Google Calendar OAuth and free/busy read, deriving the group's common free windows, calling `meetsMinimumLength` on each one (too short → `stuck` with a reason), and calling `trimPairToViableSlots` per candidate. Plus wiring the same call into `evals/adapter.ts`, which still hands over the untrimmed window.
  - **The shortest meeting worth proposing is three hours**, applied in both places: a group free window under three hours is `stuck` with a reason (`meetsMinimumLength`), and a `(venue, time)` pair narrowed under three hours is dropped — one pair, not the run (decided on [#86](https://github.com/ron14y-sys/squad_lock/issues/86)). One constant, `MINIMUM_MEETING_MINUTES`.
  - **Mobility narrows a slot, it does not forbid a venue.** Done inside the trimming, and **narrowing is on reach, never on tolerance** — being further than someone is comfortable with is a burden leximin weighs, not a wall (scenario `06`'s agreed answer puts two people at 1.15× their tolerance). On foot the reach is about a kilometre; transit and car are uncapped ([#89](https://github.com/ron14y-sys/squad_lock/issues/89)). **Deterministic here, not in A12** — the Resolver ships dark and falls back, so a Resolver-only version hands a car-sized tolerance back to someone who cannot drive every time it is off.
