...
- **`expected.time` is required whenever anything narrows the group's window** — the venue's hours, or somebody's reach — because then the answer genuinely is a `(venue, time)` pair rather than a venue. It carries a start **and** an end.
...
**The shortest meeting worth proposing is three hours, in both places it can be measured** — the group's own free window, before any venue is considered, and the slot left after a venue's hours and everyone's reach have narrowed it. Too short at the first stage is `stuck` with a reason rather than an empty shortlist; too short at the second drops one pair. Two scenarios did not when the number was agreed, and both had their availability widened rather than their venue changed: 02 from 21:00 to 20:00 (Anna Loulou still shuts at 20:00, so an intersection of zero length keeps the trap intact) and 05 from 20:30 to 19:30 (HaKosem still shuts at 22:30, so the trimmed evening is now exactly three hours).
...
| `mobilityWindows: { mode: "no-car" }`                   | `{ mode, available, window }`      | The real type is `{ mode: "car" \| "transit" \| "walk", available: boolean, window }`, so "no car" is `{ mode: "car", available: false }`                                                                                                                                                                                                                                                              |
| `reachableWithoutCar: false` on a venue                 | a mode cap on the **person**       | No restaurant is car-only; some _people_ are car-less. Reachability belongs to the journey, so it shortens a traveller's reach rather than removing a place ([#89](https://github.com/ron14y-sys/squad_lock/issues/89))                                                                                                                                                                                |
| `priceLevel: "mid-high"`, `atmosphere: "loud-bar"`      | `soft: VenueSoftFacts`             | `Candidate` carries neither, and `places.priceLevel` is Enterprise-tier alongside `rating` (spec §6.3, §13.6), so venue soft facts travel **beside** a candidate exactly as `VenueDietaryFacts` does. The vocabulary is `Partial<SoftPreferences>`: a venue answers the same four questions a person does, so matching is a field comparison rather than a translation between two invented word lists |
| `expectedConstraint: { type, category, avoid, prefer }` | `{ participant, softPreferences }` | The old shape defined no type, named no owner, and used words the venues did not use. A rejection is a per-meeting correction on a `ParticipantMeetingContext` row, which carries the `userId` and the meeting — answering both questions at once. "Too expensive this time" does not make somebody permanently thrifty                                                                                |

**A soft preference is optional, and an absent one changes nothing.** Not having an opinion about noise is a real state, and it is not the same as wanting either answer — so an absent field may never cost a venue a place or win it one. That is what makes 05 writable: Hila is the only person there who stated anything, and the other three must not become three silent votes for the better-rated option. The deterministic column already behaves this way, because nothing in it branches on `softPreferences` at all; the obligation falls on A4's prompt and A6's justification check. C2 needs a way to decline a question rather than forcing four answers.
...
| 3   | `03-mobility-window-trap.json`                | mobility-window     | A participant is on foot for the first part of the evening, so the fairest venue is out of reach until 20:00 — the answer is a `(venue, time)` pair, not a venue. |
...
Fixing the scenarios in [#86](https://github.com/ron14y-sys/squad_lock/issues/86) turned up six things that were design decisions rather than fixture work. All six are answered in that issue and implemented here.

| #   | Question                                                           | Answer                                                                                                                                                                       |
| --- | ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `priceLevel` and `atmosphere` had nowhere to live                  | `VenueSoftFacts`, passed **beside** a candidate as `VenueDietaryFacts` is, typed `Partial<SoftPreferences>` so venue and person share one vocabulary                         |
| 2   | How a venue that needs a particular way of travelling is expressed | It is not. Travel mode caps how far a **person** can reach — on foot about a kilometre, transit and car uncapped ([#89](https://github.com/ron14y-sys/squad_lock/issues/89)) |
| 3   | What happens to scenario 03                                        | Rewritten around a walking-only participant, restoring spec §9's mobility-window trap                                                                                        |
| 4   | What `expectedConstraint` really is                                | A per-meeting correction on a `ParticipantMeetingContext` row — `{ participant, softPreferences }`                                                                           |
