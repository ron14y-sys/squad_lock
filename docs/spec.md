...
| LLM       | Gemini API (`@google/genai`)       | See §6.4                                                   |
...
| 3   | **Gemini API**              | Matching agent; flash-class extraction models (§6.4)         | API key                                   | Free tier, with a daily cap that may force the paid tier — §6.4   |
...
All secrets — the Places key, the Gemini key, the email provider key, the OAuth client secret, the database URL — live as **Vercel environment variables**, never in the repository (§10).
...
> ⚠️ **The provider changed on 2026-08-26, from Anthropic to Google.** Earlier drafts of this section specified Claude, and every `claude-*` model named in them is gone from this project. The reason is not quality: **the Gemini API has a free tier and the Anthropic API does not**, and a Claude Pro or Max subscription buys the apps, not API credits. Measured evidence for what follows is in [docs/decisions/runtime-budget.md](decisions/runtime-budget.md).

| Model                 | ID                      | Input / Output per 1M tokens (paid tier)              | On the free tier |
| --------------------- | ----------------------- | ----------------------------------------------------- | ---------------- |
| Gemini 3.5 Flash-Lite | `gemini-3.5-flash-lite` | $0.30 / $2.50                                         | Yes              |
| Gemini 3.6 Flash      | `gemini-3.6-flash`      | $0.75 / $3.75 (rising to $1.50 / $7.50 on 2027-01-01) | Yes              |
| Gemini 3.5 Flash      | `gemini-3.5-flash`      | $1.50 / $9.00                                         | Yes              |

Prices read from Google's [pricing page](https://ai.google.dev/gemini-api/docs/pricing) on 2026-08-26. **Re-check before quoting any of them in the report** — this document has been wrong twice by inheriting a provider's numbers rather than reading them.

**The free tier is flash-class only.** Every pro-class model returns a quota of **0** to a new free-tier key — `gemini-2.5-pro` answers 404 "no longer available to new users", and `gemini-3.1-pro-preview` reports a zero quota. There is no strong-model option to evaluate against while the project stays free, which makes model choice a decision between flash models for as long as the project stays free.

**Mapping:** `gemini-3.5-flash-lite` for development and repeated runs · `gemini-3.6-flash` where the extra reasoning earns its cost · a pro-class model only if evaluation proves a gap **and** the project has moved to the paid tier.

The **Context Resolver** (§4) and the **Constraint Updater** are twins: both are small extraction tasks turning free text into a validated typed object, both sit on `gemini-3.5-flash-lite` from the start, and both share the same prompt, schema, and validation conventions — which is why one person builds both.

The matching agent is the one place where model choice actually matters — it does all the reasoning in the system, and there is no second agent to catch its mistakes. **Measured behaviour, twenty runs over the full six-participant payload: every configuration returned three ranked options and justified every participant, with no omissions.** That is the first evidence against the §9 risk that one agent holding six profiles quietly drops one, and it is not a substitute for A6's dedicated check.

**Latency is the real constraint, not cost.** Worst observed run: **4.7s** on `gemini-3.5-flash-lite` at thinking `low`, deployed; **207.8s** on `gemini-3.6-flash` at thinking `high`, locally. Against Vercel Hobby's 300s function limit that is 2% and 69% respectively — so the model and thinking level together decide whether the run has margin. Success criterion 6 (≤ 20s) is met by `gemini-3.5-flash-lite` at `low` and by nothing else measured.

**`thinking_level` is Gemini's effort dial** — `minimal` / `low` / `medium` / `high`. Routine matching at `low`; higher settings only where a trade-off case earns the wait. ⚠️ **Thinking tokens are spent from `max_output_tokens`, and they dwarf the answer** — a measured run spent 24,299 thought tokens against 1,566 of output. An output cap sized for the answer alone truncates the response mid-JSON, silently. Size it for both, and treat a stream that ends without a completion event as truncated rather than malformed.

⚠️ **The free tier does not have the throughput this project needs.** `gemini-3.6-flash` allows **20 requests per day** (`GenerateRequestsPerDayPerProjectPerModel-FreeTier`); one `npm run eval` over the F5 scenarios is most of that. Free-tier prompts are also used to improve Google's products, which conflicts with §10 once real participants' constraints and calendars are involved. **Moving to the paid tier is an open decision with a deadline, not a preference** — see §13, item 17.
...
| 17  | **Free tier or paid tier for the Gemini API?**                                                       | Open, and it has a deadline. The free tier caps `gemini-3.6-flash` at **20 requests per day**, which one eval run exhausts (§6.4), and free-tier prompts are used to improve Google's products, which §10 does not permit for real participants' constraints and calendars. Both point the same way; what is undecided is when, and who pays |
